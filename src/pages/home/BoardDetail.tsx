...
export default function BoardDetail() {
  ...
  const loadBoardData = async (boardId: string) => {
    try {
      ...
      setBoardMembers(
        (boardData.members || []).map((m: any) =>
          typeof m === "string"
            ? { _id: m, username: "Không rõ", email: "Không rõ" }
            : m
        )
      );
  ...
  const handleSendInvite = async () => {
    ...
    try {
      const updatedBoard = await boardsApi.inviteMemberByEmail(board._id, inviteEmail);
      
      setBoard(updatedBoard);
      setBoardMembers(
        (updatedBoard.members || []).map((m: any) =>
          typeof m === "string"
            ? { _id: m, username: "Không rõ", email: "Không rõ" }
            : m
        )
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full animate-slideUp">
            <div className="p-6">
              ...
              {board && (board as any).createdBy && (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl">
                  <p className="text-sm text-blue-900 font-medium">
                    Người tạo bảng: <span className="font-bold">{(board as any).createdBy}</span>
                  </p>
                </div>
              )}
